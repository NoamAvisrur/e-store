class Review{    
    setReview(arr){
        ...
        for (let i = 0; i < reviews_buttons.length; i++){
            reviews_buttons[i].addEventListener('click', function(e){
                ...
                Review.prototype.submitReview(arr, i);
            });
        }
    }
        ...
        if(document.querySelector('form')){
            document.querySelector('form').addEventListener('submit', function(e){
                ...
                if (document.querySelector('input[type="text"]').value !== "" && typeof Review.prototype.index != 'undefined'){
                    ...
                    console.log(i);
                    Review.prototype.buildComments(comment, rate, e.target, data, i);
    ...
    buildComments(comment, rate, parent, data, i){
        Review.prototype.pushCommetsToData(comment, rate, data, i);
        var review_container = document.createElement('div');
        review_container.className = "review_container";
        var text = document.createElement ('span');
        text.className = "comment";
        text.textContent = comment + Review.prototype.getCommentDate();
    ...
    pushCommetsToData(comment, rate, data, i){
        console.log(i);
        data[i].review.unshift(
            {
